...
function is_special_char( nameV) {
	try{
		...
		alert( "is_special_char( '"+nameV+"' ) 함수에서 에러발생");
		return false;
	}
}

// 입력양식에 value 값을 삽입하거나 체크해 주는 함수선언
// 매개변수로 입력양식의 name 값과 입력값이 들어온다. 
function inputData( nameV, data ) {
	try {
		// 만약 data가 null 또는 undefined 면 함수 중단하기
		if(data==null || data==undefined) { return; }
		// 만약 길이가 없거나 공백으로만 구성되어 있으면 함수 중단
		if( (data+"").split(" ").join("")=="" ) { return; }
		// name 값을 가진 입력 양식을 관리하는 JQuery 객체 생성
		var obj = $( "[name="+nameV+"]");
		...
		// 만약 입력 양식이 checkbox 또는 radio 가 아니면
		else {
			obj.val(data);
		}
	} catch(e) {
...
// 입력양식에 value 값을 삭제하거나 체크를 풀어주는 함수선언
function setEmpty( nameV ) {
	try {
		...
		//-----------------------------------------------------------------
		// 만약 입력 양식이 checkbox 또는 radio면 naveV 변수 안의 데이터를 가진 checkbox 또는 radio를 체크 풀기
		if (obj.is(":checkbox") || obj.is(":radio") ) {
			obj.prop("checked", false );
		}
		// 만약 입력 양식이 checkbox 또는 radio 가 아니면 nameV 안의 데이터 제거하기
		else {
			obj.val("");
		}
	} catch(e) {
		alert( "setEmpty( '"+namev+"', ~) 함수에서 에러발생" );
	}
}
...
function is_pattern( nameV, patternObj ) {
	try {
		// 이름에 해당하는 입력양식을 관리하는 JQuery 객체 생성하기  
		var jqObj = $("[name='"+nameV+"']");
		//checkbox, radio, select 입력양식을 관리하는 JQuery 객체 생성하기
		if ( jqObj.is(":checkbox") || jqObj.is(":radio") || jqObj.is("select") ) {
			alert("checkbox 또는 radio 또는 select 는 is_pattern 함수의 호출대상이 아닙니다");
			return;
		}
		//alert( 11);
		var value = jqObj.val();
...
function setTableTrBgColor(
		...
		, headerColor				// 헤더 배경색
		, oddBgColor				// 홀수 tr 배경색
		, evenBgColor				// 짝수 tr 배경색
		, mouseOnBgColor		// 마우스 on 할때 배경색
) {
	try {
